...
def read_json(data_location):
    ...
    for file in os.listdir(directory):
        ...
        this_dataset_by_relation = eval(dataset)
        ...
        for x in data:
            ...
            relation = x['relation']
            this_small_dict = {"tokens": tokens,"h":[head,None,[[i for i in range(head_start,head_end+1)]]]
                ,"t":[tail,None,[[i for i in range(tail_start,tail_end+1)]]]}
            insert_to_dict_data(all_data_dict_per_relation_list, relation, this_small_dict)
            insert_to_dict_data(this_dataset_by_relation, relation, this_small_dict)
...
no_rel = all_data["no_relation"]
filtered_no_rel = random.sample(no_rel,15)
from pprint import pprint
pprint(filtered_no_rel)
